...
%% Weighting functions
s = tf('s');
Wd_tf = 1*(0.01/5)*((s+5)/(s+0.01));    % (default: 1)
Wr_tf = 10*(0.01/5)*((s+5)/(s+0.01));   % (default: 10)
Wu_tf = 05*(10/0.5)*((s+0.01)/(s+5));   % (default: 05)
% Wd_tf = 0.75*((s+16)/(s+0.01));
% Wr_tf = 0.75*((s+16)/(s+0.01));
% Wu_tf = 0.485*((s+0.2)/(s+1400));
...
% figure(1);
% bode_handle = bodeplot((Wd_dis)^-1,'-k',(Wr_dis)^-1,'--k',(Wu_dis)^-1,'-.k');
% setoptions(bode_handle,'PhaseVisible','off','grid','on','Xlim',[10^-4,30]);
% legend('Wd^{-1}','Wr^{-1}','Wu^{-1}');

%% Plant (Spark ignite automative engin)
Ap = [-49.2400 -1.3230
       59.0484 -2.5325];
...
sys_ss = ss(Ap,Bp_u,Cp,Dp);

sys_dis_ss = c2d(sys_ss,T,'zoh');
...
p = 50; % order of past data (default: 30)
f = 55;  % order of future data (default: 70)

gamma_min = 0.250871;   % (default: 0.301217)
...
fprintf('*\t\tcopy above value to line 111 and run again!\t\t*\n');
fprintf('*********************************************************\n\n');
...
% setPoint = (180/pi)*2*ones(nSteps+1,1);
% setPoint = (180/pi)*[0*ones(nSteps/4,1);2*ones(nSteps/2,1);1.5*ones(nSteps/4 + 1,1)];
setPoint = (180/pi)*(0*(time<=round(t_f/10)) + 2*(time>round(t_f/10) & time<=round(t_f/1.3)) + 1.5*(time>round(t_f/1.3)));
setPoint = [setPoint setPoint(end)]';

% noise = randn(nSteps+1,1);
noise = filter([1 -alpha],[1 -1],noise);

dist = 0*(time<=round(t_f/2.7)) + 1.5*(time>round(t_f/2.7) & time<=round(t_f/1.8)) + 0*(time>round(t_f/1.8));
dist = [dist 0]';
% dist = 0*(time<=round(t_f/3)) + 1.5*(time>round(t_f/3));
% dist = [dist 0]';
...
%    alpha_2 = 0;
%    r = setPoint(k)*ones(f,1);
%    w = alpha_2*w + (1-alpha_2)*r;
%    smoothed_ref(k) = w(1); 
...
%    yp(k+1) = -Denominator*yp_past + Numerator*up_past;
    ...
    xp(:,k+2) = Ap_z*xp(:,k+1) + Bp_z*uf(k+1) + 5*[0;Bp_z(2,1)]*dist(k+1);
...
% y_p = simulate_plant(uf,nSteps);
y_openLoop = step(sys_dis_ss,time);
% y_openLoop = step(sys_dis,time);
% [integrated_noisy_y_openLoop] = add_noise_func(y_openLoop,alpha,snr_dB);
...
%% Control inputs
figure(2);
plot(time,uf,'b','LineWidth',1);
grid on;
xlabel('\bfTime (s)');
ylabel('\bfu(t)');
legend('\bfProposed','Location','NorthEast');

%% Prediction error
figure(3);
plot(time,deg2rad*(yp-yf),'b','LineWidth',1);
ylim([-2 2]);
grid on;
xlabel('\bfTime (s)');
ylabel('\bfe_p(t)');
legend('\bfPrediction error');
title('\bfProposed');

%% Prediction results
figure(4);
plot(time,deg2rad*yp,'r--',time,deg2rad*yf,'b');
grid on;
xlabel('Time (s)');
ylabel('Output (rad/sec)');
legend('Plant output','Predicted output','Location','SouthEast');
title('Proposed');
h4 = gca;
h4.Children(1).LineWidth = 1;
h4.Children(2).LineWidth = 3;

%% Tracking errors
figure(5);
plot(time,deg2rad*(setPoint-yp),'b','LineWidth',1);
ylim([-2 2]);
grid on;
xlabel('Time (s)');
ylabel('e_t(t)');
...
%% Tracking results
figure(6);
plot(time,deg2rad*setPoint,'r--',time,dist,'k-.',time,deg2rad*yp,'b');
grid on;
ylim([-0.2,3.5]);
xlabel('Time (s)');
ylabel('Output (rad/sec)');
legend('Reference signal','Disturbance',...
       'Closed-loop response',...
       'Orientation','Vertical','Location','NorthEast');
title('Proposed');
h5 = gca;
h5.Children(1).LineWidth = 1;
h5.Children(2).LineWidth = 1;
h5.Children(3).LineWidth = 1.5;
